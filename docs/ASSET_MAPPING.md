...
**Purpose:** Document which spritesheet contains what, and recommend mappings to spells/monsters that match the current runtime asset setup.

**Runtime Asset Locations:**
- Spell effects: `C:\Users\danie\Documents\New project\frontend\public\assets\spelleffects\` (`1010.png`, `1011.png`, etc)
- Monster sprites: `C:\Users\danie\Documents\New project\frontend\public\assets\monsters\` (`00.png`-`59.png`)

**Important:**
- This file should only describe assets currently used by the Phaser runtime.
- Do not use old `monstersprites/` or `0x72` references here unless the loader is changed back to support them.
- Boss sprite files are reserved for bosses only and should not be reassigned to regular monsters.
...
- **1022.png** -> Similar to `1020.png`

- **1023.png** -> Similar to `1020.png`
...
- **04.png** -> Dark/maroon humanoids (variant of `01.png`?)
  - Similar structure to `01.png` but possibly different animation angles

- **10.png** -> Dark humanoids with slight variations

- **20.png** -> Brown humanoids (variant color of same type as `01.png`)
  - Appears to be the same character model, different color

### Observation
Monster files seem to follow a pattern where **each number represents a different color/variant of the same base model**, rather than completely different monsters. Files `01`, `04`, `10`, and `20` appear to be the same humanoid unit type in different colors/conditions.
...
**Support Spells:**
- Visual effects typically do not need custom spritesheets; use existing `FX_ANIMS` or reuse main spell effects.
...
**Boss-reserved files:** `01`, `04`, `07`, `09`, `11`, `16`, `17`, `18`, `21`, `25`  
Do not assign these spritesheets to regular monsters.

### Regular Monster Assignments (Freely Chosen)

**Existing regular monsters -> Runtime sprites:**
...
- **blazeWarden** -> **24.png** (armored humanoid with red/orange accents; best current non-boss fit for a fire-aligned rare melee warden)
...
**Blaze Warden fallback candidates (non-boss only):**
- **24.png** -> best match; armored melee silhouette with hot accent colors
- **15.png** -> stronger demon/fire look, but less "warden" and more infernal creature
- **22.png** -> usable elite silhouette, but reads more regal/brute than fire guardian
...
- **Spell effects:** 64x64 frame size, 6-8 columns x multiple rows (animation progression)
